...
export default function EditEmployee() {
  ...
  useEffect(() => {
    ...
    // console.log(props);
    axios.get(`/users/${e_id}`)
    .then(respnse =>{
        if(respnse.data!=null){
            setEmployee(respnse.data);
...
const updateEmployee= (e) =>{
  ...
  axios.patch(`/users/${e_id}`, data)
  .then(res => 
  {
    if (res.data !=null) {
      swal("Success",'ress','success');
...
if (loading) {
    return <h1>Loading Edit  user.......</h1>
  }
  return (
    ...
    <h4>Update Employee
    <Link to="/employee" className="btn btn-primary btn-sm float-end"> View User</Link>
    </h4>
